...
def _readonly_text(parent, testo: str, font, bg=None, fg=None):
    """
    Crea un widget tk.Text in sola lettura che si espande in altezza quanto
    serve per mostrare tutto il contenuto, senza scrollbar e senza tagliare.
    Si ricalcola ogni volta che il contenitore cambia larghezza.
    """
    if bg is None:
        bg = ENTRY_BG
    if fg is None:
    ...
    txt = tk.Text(parent, font=font,
                  ...
                  height=1)          # altezza iniziale minima
    txt.insert("1.0", testo)
    txt.config(state="disabled")

    def _fit_height(event=None):
        """Ricalcola l'altezza in righe visualizzate dopo ogni resize."""
        txt.update_idletasks()
        # count("update") aggiorna il layout interno prima della misura
        txt.config(state="normal")
        # Misura le righe di display (non le righe logiche)
