...
class ListWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Button(top_inner, text="← Indietro",
                  font=self.f_btn, bg=ENTRY_BG, fg=TEXT_MID,
                  activebackground=ENTRY_BD, activeforeground=TEXT_DARK,
                  relief="flat", bd=0, cursor="hand2",
                  padx=14, pady=6, command=self._on_back).pack(side="right")
...
class DetailWindow(tk.Toplevel):
    ...
    def _build_ui(self, item: dict):
        ...
        tk.Label(root, text="Ringraziamenti",
                 font=self.f_title, bg=BG, fg=TEXT_DARK).pack()
        tk.Label(root, text=f"Per {item['destinatario']}  ·  {item['data']}",
                 font=self.f_sub, bg=BG, fg=TEXT_MID).pack(pady=(4, 20))
        ...
        tk.Label(inner, text=item["destinatario"],
                 font=self.f_dest, bg=CARD_BG, fg=ACCENT, anchor="w").pack(fill="x")
        ...
        tk.Label(inner, text=item["testo"],
                 ...
                 wraplength=360, justify="left", anchor="nw").pack(fill="x")
        ...
        code_pill.pack(anchor="w", pady=(6, 0))
        tk.Label(code_pill, text=item["codice"],
                 font=self.f_code, bg=ENTRY_BG, fg=ACCENT,
                 padx=16, pady=8).pack()

        # Bottone indietro
        tk.Button(root, text="← Torna alla lista",
                  font=self.f_btn, bg=ENTRY_BG, fg=TEXT_MID,
                  activebackground=ENTRY_BD, activeforeground=TEXT_DARK,
                  relief="flat", bd=0, cursor="hand2",
