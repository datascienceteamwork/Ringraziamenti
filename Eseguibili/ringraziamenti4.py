...
def _get_work_area():
    """
    Restituisce (x, y, larghezza, altezza) dell'area di lavoro disponibile
    escludendo la barra delle applicazioni di Windows.
    Su altri OS ricade sulle dimensioni dello schermo intero.
    """
    try:
        import ctypes
...
class RingraziamentiApp(tk.Tk):
    def __init__(self):
        ...
        # Calcola area disponibile tenendo conto della taskbar
        work = _get_work_area()
        if work:
            wx, wy, ww, wh = work
        else:
        ...
        win_w = 460
        x = wx + (ww - win_w) // 2
        self.geometry(f"{win_w}x{wh}+{x}+{wy}")
