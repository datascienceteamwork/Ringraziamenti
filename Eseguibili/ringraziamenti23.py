...
def _readonly_text(parent, testo: str, font, bg=None, fg=None, height=None):
    """
    Crea un widget tk.Text in sola lettura con wrap word e margini interni,
    che si adatta alla larghezza del contenitore senza tagliare le parole.
    L'altezza viene calcolata automaticamente se non specificata.
    """
    if bg is None:
        bg = ENTRY_BG
    if fg is None:
    ...
    if height is not None:
        ...
        # Calcola l'altezza in righe necessaria dopo il rendering
        def _auto_height(event=None):
            txt.config(state="normal")
            txt.update_idletasks()
            # Conta le righe visualizzate
            last_idx = txt.index("end-1c")
            rows = int(txt.index(f"@0,{txt.winfo_height()}").split(".")[0])
            # Usa dlineinfo per contare le righe visibili reali
            line_count = int(txt.index("end").split(".")[0])
...
class ListWindow(tk.Toplevel):
    ...
    def _make_card(self, parent, item: dict):
        ...
        # Badge risposta ricevuta
        if item.get("risposta", ""):
            badge_row = tk.Frame(inner, bg=CARD_BG)
            badge_row.pack(fill="x", pady=(8, 0))
            badge = tk.Frame(badge_row, bg="#E8F0E9",
                             highlightthickness=1, highlightbackground="#A8CCA9")
    ...
    def _build_ui(self, item: dict):
        ...
        if risposta:
            ...
            _readonly_text(resp_inner, risposta,
                          font=self.f_thanks).pack(fill="x")
...
class ThanksWindow(tk.Toplevel):
    ...
    def _build_ui(self, destinatario: str, testo: str, risposta: str, autore: str):
        ...
        root = tk.Frame(scroll_frame, bg=BG)
        root.pack(fill="x", padx=30, pady=20)
        ...
        resp_inner = tk.Frame(resp_card, bg=CARD_BG)
        resp_inner.pack(fill="x", padx=28, pady=24)
        ...
        if risposta:
            # Risposta già inviata: mostrala in sola lettura
            _readonly_text(resp_inner, risposta,
                          font=self.f_thanks).pack(fill="x")
            tk.Label(resp_inner,
            ...
            # Nessuna risposta: mostra textarea + bottone invia
            tk.Label(resp_inner, text="SCRIVI LA TUA RISPOSTA",
                     font=self.f_label, bg=CARD_BG, fg=TEXT_MID).pack(anchor="w")

            text_frame = tk.Frame(resp_inner, bg=ENTRY_BG,
                                  highlightthickness=1, highlightbackground=ENTRY_BD)
            text_frame.pack(fill="x", pady=(6, 0))
            self._text_resp = tk.Text(
                text_frame, font=tkfont.Font(family="Helvetica", size=11),
                bg=ENTRY_BG, fg=TEXT_LIGHT,
                relief="flat", bd=0, wrap="word",
                insertbackground=TEXT_DARK, height=5,
                padx=10, pady=8)
            ...
            self._lbl_resp_msg = tk.Label(resp_inner, text="",
                                          font=self.f_small, bg=CARD_BG,
                                          fg=RED_ERR, wraplength=340)
    ...
    def _on_invia_risposta(self):
        ...
        threading.Thread(
            target=self._do_invia_risposta,
    ...
    def _risposta_inviata(self, testo_resp: str):
        """Aggiorna l'UI dopo l'invio: nasconde form, mostra testo confermato."""
        # Distruggi tutti i widget figli del frame risposta e mostra conferma
        parent = self._btn_invia.master
        for w in parent.winfo_children():
            w.destroy()
        _readonly_text(parent, testo_resp,
                      font=self.f_thanks).pack(fill="x")
        tk.Label(parent,
