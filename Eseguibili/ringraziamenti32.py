...
class RingraziamentiApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        lbl_forgot = tk.Label(self.forgot_frame, text="Password dimenticata?",
                 font=self.f_link, bg=CARD_BG, fg=ACCENT,
                 cursor="hand2")
        lbl_forgot.pack(side="right")
        lbl_forgot.bind("<Button-1>", lambda e: self._on_forgot_password())
    ...
    def _make_entry(self, parent, placeholder, show=None):
        ...
        # Icona occhio (solo per campi password)
        if show:
            _visible = [False]   # stato: False=nascosto, True=visibile
            eye_lbl = tk.Label(frame, text="👁",
        ...
        e.pack(side="left", fill="x", expand=True, padx=(10, 0), pady=8)
        ...
        if show:
            def toggle_visibility(_=None):
                ...
                if _visible[0]:
                    e.config(show="")
                    eye_lbl.config(text="🙈", fg=ACCENT)
                else:
                    e.config(show=e.real_show)
                    eye_lbl.config(text="👁", fg=TEXT_LIGHT)
        ...
        def on_focus_in(_):
            if e.showing_placeholder:
                ...
                if e.real_show and not (show and _visible[0]):
                    e.config(show=e.real_show)
    ...
    def _on_forgot_password(self):
        """Apre un popup modale per il recupero password via Firebase."""
        popup = tk.Toplevel(self)
        popup.title("Recupero password")
        popup.resizable(False, False)
        popup.configure(bg=CARD_BG)
        popup.grab_set()
        ...
        f_title = tkfont.Font(family="Helvetica", size=13, weight="bold")
        f_body  = tkfont.Font(family="Helvetica", size=10)
        f_label = tkfont.Font(family="Helvetica", size=10, weight="bold")
        f_btn   = tkfont.Font(family="Helvetica", size=11, weight="bold")
        f_small = tkfont.Font(family="Helvetica", size=9)
        f_entry = tkfont.Font(family="Helvetica", size=11)

        outer = tk.Frame(popup, bg=CARD_BG)
        outer.pack(fill="both", expand=True, padx=28, pady=24)

        tk.Label(outer, text="\U0001f510  Recupero password",
                 font=f_title, bg=CARD_BG, fg=TEXT_DARK).pack(anchor="w", pady=(0, 8))
        tk.Label(outer,
                 text="Inserisci la tua email: ti invieremo un link per reimpostare la password.",
                 font=f_body, bg=CARD_BG, fg=TEXT_MID,
                 wraplength=324, justify="left").pack(anchor="w", pady=(0, 14))

        tk.Label(outer, text="EMAIL", font=f_label,
                 bg=CARD_BG, fg=TEXT_MID).pack(anchor="w")

        entry_frame = tk.Frame(outer, bg=ENTRY_BG,
                               highlightthickness=1, highlightbackground=ENTRY_BD)
        entry_frame.pack(fill="x", pady=(4, 0))
        entry_email = tk.Entry(entry_frame, font=f_entry,
                               bg=ENTRY_BG, fg=TEXT_LIGHT,
                               relief="flat", bd=0,
                               insertbackground=TEXT_DARK)
        entry_email.pack(fill="x", padx=10, pady=8)
        entry_email.insert(0, "Es. mario@email.com")
        _ph_active = [True]

        def ef_in(_):
            if _ph_active[0]:
                entry_email.delete(0, "end")
                entry_email.config(fg=TEXT_DARK)
                _ph_active[0] = False
        ...
        def ef_out(_):
            if entry_email.get() == "":
                entry_email.insert(0, "Es. mario@email.com")
                entry_email.config(fg=TEXT_LIGHT)
                _ph_active[0] = True
            entry_frame.config(highlightbackground=ENTRY_BD)

        entry_email.bind("<FocusIn>",  ef_in)
        entry_email.bind("<FocusOut>", ef_out)

        lbl_msg = tk.Label(outer, text="", font=f_small,
                           bg=CARD_BG, fg=RED_ERR, wraplength=324)
        lbl_msg.pack(fill="x", pady=(8, 0))

        btn_row = tk.Frame(outer, bg=CARD_BG)
        btn_row.pack(fill="x", pady=(10, 0))

        def invia():
            if _ph_active[0] or not entry_email.get().strip():
                lbl_msg.config(text="Inserisci la tua email.", fg=RED_ERR)
                return
            email = entry_email.get().strip()
            btn_send.config(state="disabled")
            lbl_msg.config(text="Invio in corso…", fg=TEXT_MID)
            threading.Thread(target=_do_reset, args=(email,), daemon=True).start()
        ...
        def _do_reset(email: str):
            try:
                ...
                with urllib.request.urlopen(req):
                    pass
                popup.after(0, _reset_ok)
            except urllib.error.HTTPError as e:
                err = _json.loads(e.read()).get("error", {}).get("message", "")
                if err in ("EMAIL_NOT_FOUND", "INVALID_EMAIL"):
                    msg = "Nessun account trovato con questa email."
                else:
                    msg = f"Errore: {err}"
                popup.after(0, lambda: (
                    lbl_msg.config(text=msg, fg=RED_ERR),
                    btn_send.config(state="normal")
                ))
            except Exception as ex:
                popup.after(0, lambda: (
                    lbl_msg.config(text=f"Errore: {ex}", fg=RED_ERR),
                    btn_send.config(state="normal")
                ))
        ...
        def _reset_ok():
            for w in outer.winfo_children():
                w.destroy()
            tk.Label(outer, text="\u2705  Email inviata!",
                     font=f_title, bg=CARD_BG, fg=BTN_READER_FG).pack(pady=(10, 10))
            tk.Label(outer,
                     text="Controlla la tua casella di posta e segui il link per reimpostare la password.",
                     font=f_body, bg=CARD_BG, fg=TEXT_MID,
                     wraplength=324, justify="center").pack(pady=(0, 20))
        ...
        btn_send = tk.Button(btn_row, text="Invia link di recupero",
                             font=f_btn, bg=ACCENT, fg="white",
                             activebackground=ACCENT_DARK, activeforeground="white",
                             relief="flat", bd=0, cursor="hand2",
                             pady=9, command=invia)
        btn_send.pack(fill="x", pady=(0, 8))

        tk.Button(btn_row, text="Annulla",
                  font=f_btn, bg=ENTRY_BG, fg=TEXT_MID,
                  activebackground=ENTRY_BD, activeforeground=TEXT_DARK,
                  relief="flat", bd=0, cursor="hand2",
                  pady=9, command=popup.destroy).pack(fill="x")
