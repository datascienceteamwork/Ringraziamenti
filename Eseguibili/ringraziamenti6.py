...
class ReaderWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        # ── Sezione testo ringraziamento (inizialmente nascosta) ───────────────
        self.thanks_card = tk.Frame(root, bg=CARD_BG,
                                    highlightthickness=1,
                                    highlightbackground=ENTRY_BD)

        thanks_inner = tk.Frame(self.thanks_card, bg=CARD_BG)
        thanks_inner.pack(fill="both", expand=True, padx=28, pady=24)

        # Intestazione con nome scrittore
        top_row = tk.Frame(thanks_inner, bg=CARD_BG)
        top_row.pack(fill="x", pady=(0, 14))
        tk.Label(top_row, text="✉  Messaggio per te",
                 font=tkfont.Font(family="Helvetica", size=12, weight="bold"),
                 bg=CARD_BG, fg=ACCENT).pack(side="left")

        # Linea separatrice
        tk.Frame(thanks_inner, bg=ENTRY_BD, height=1).pack(fill="x", pady=(0, 16))

        # Testo ringraziamento (wrappato)
        self.lbl_thanks = tk.Label(
            thanks_inner, text="",
            font=self.f_thanks,
            bg=CARD_BG, fg=TEXT_DARK,
            wraplength=360, justify="left", anchor="nw")
        self.lbl_thanks.pack(fill="x")

        # Firma
        self.lbl_author = tk.Label(
            thanks_inner, text="",
            font=tkfont.Font(family="Helvetica", size=10, slant="italic"),
            bg=CARD_BG, fg=TEXT_LIGHT, anchor="e")
        self.lbl_author.pack(fill="x", pady=(14, 0))
    ...
    def _on_confirm(self):
        ...
        # Simulazione: qualsiasi email + codice valido mostra un testo di esempio
        self.lbl_msg.config(text="✓ Ringraziamento trovato!", fg=BTN_READER_FG)
        ...
        self.lbl_thanks.config(text=testo_esempio)
        self.lbl_author.config(text=f"— {email.split('@')[0].capitalize()}")

        # Mostra la card con il testo
        self.thanks_card.pack(fill="x", pady=(16, 0),
                               before=self.thanks_card.master.winfo_children()[-1])
